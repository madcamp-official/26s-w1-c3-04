...
- [ ] URL 해시 기반 중복 기사 제거
- [ ] 시장/기업 영향력 스코어링 (실적/재무, 지배구조/경영권, 계약/수주, 리스크, 자본변동, 시장반응 6개 카테고리 기준, 마케팅·PR·이벤트성 기사는 낮은 점수)
- [ ] 일반 피드와 구독(종목/분야) 피드 모두 동일한 스코어링 기준 적용
- [ ] 임계값 동적 하향 (7점 시작 → 소진 시 6점, 5점까지 하향 / 5점 밑 마케팅·PR 구간은 어떤 상황에서도 노출 안 함)
- [ ] 5점까지 내려도 새 기사가 없으면 "지금은 여기까지예요, 새로운 뉴스를 기다려주세요" 안내 카드 표시
- [ ] LLM 기반 핵심 요약 생성 (헤드라인 + 3~4문장)

2. 종목 및 분야 태깅
- [ ] 종목 태깅: 화이트리스트 매칭 + 판단 기준(헤드라인·리드문 등장 → 사건의 주체 여부 → 기사당 최대 2개)
- [ ] 분야 태깅: 반도체·2차전지 등 산업 분야 분류, 기사당 최대 2개
- [ ] 뉴스 카드에 종목 태그 + 분야 태그 함께 표시

3. 구독 시스템
- [ ] 회사 구독: 검색 화면에서 추가/해제
- [ ] 분야 구독: 좌측 슬라이드 패널에서 추가/해제, 분야 탭하면 해당 분야 뉴스로 필터링
- [ ] 카드의 종목·분야 태그를 눌러 그 자리에서 바로 구독 추가/해제

4. 홈 화면
- [ ] 상단 구독 스토리 레일 (안 읽음 = 빨간 링, 확인함 = 회색 링)
- [ ] 스토리 탭 → 해당 종목 뉴스만 숏츠로 필터링
- [ ] 좌측 "관심 분야" 패널 진입 버튼
- [ ] 영향력 점수순 단일 피드 (오늘의 화제 뉴스)

5. 피드 인터랙션
- [ ] 세로 스와이프, 당겨서 새로고침
- [ ] 종목 태깅된 카드는 좌우 스와이프로 해당 기업 차트 전환 (탭 버튼 병행)
- [ ] 좋아요 / 스크랩
- [ ] 이미지 없는 기사는 카테고리별 대체 아이콘

6. 차트
- [ ] KIS Open API(모의투자 계좌, 팀 명의 1개) 연동, 당일 장중 시세 반영
- [ ] 하단 탭 진입 시 구독 종목 워치리스트 → 종목 선택 시 차트
- [ ] 뉴스 카드 스와이프로 진입 시 해당 종목 차트로 바로 진입
- [ ] 두 진입 경로 모두 동일한 차트 화면 재사용, 진입 경로에 따라 뒤로가기/탭 하이라이트 분기
- [ ] 일 / 주 / 월 기간별 시세 차트
...
- [ ] 난이도별 요약 (입문자용 / 전문가용)
- [ ] TTS 음성 읽기
- [ ] 스와이프 마이크로 인터랙션 (스프링 애니메이션, 진동 피드백 등)
